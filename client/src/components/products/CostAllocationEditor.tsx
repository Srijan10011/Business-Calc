...
export default function CostAllocationEditor() {
    const [rules, setRules] = React.useState<CostRule[]>([
    ...
    const handleAddRule = async () => {
        try {
            ...
            const newRule = { id: nextId, category: '', type: '', mode: 'percentage' as const, value: '' };
    ...
    const handleSaveRule = async (rule: CostRule) => {
        try {
            const token = localStorage.getItem('token');
    ...
    const handleRuleChange = (id: number, field: keyof CostRule, value: any) => {
                        ...
                        <TableRow key={rule.id}>
                            ...
                            <TableCell>
                                <FormControl variant="standard" sx={{ minWidth: 120 }}>
                                    ...
                                    <Select
                                        value={rule.mode}
                                        onChange={(e) => handleRuleChange(rule.id, 'mode', e.target.value as 'percentage' | 'fixed')}
                                        label="Mode"
                                    >
                            ...
                            <TableCell align="right">
                                ...
                                <IconButton onClick={() => handleRemoveRule(rule.id)} color="error">
                                    <DeleteIcon />
                                </IconButton>
                            </TableCell>
                        </TableRow>
